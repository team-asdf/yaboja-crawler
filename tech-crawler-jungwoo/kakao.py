...
def getLinks():
    ...
    try:
        f = open(os.path.dirname(os.path.realpath(__file__)) + "/data/kakao.csv", "r", encoding='utf-8')
        rdr = csv.reader(f)
        for line in rdr:
            already_list.append(line[2])
        f.close()
    except FileNotFoundError:
        f = open(os.path.dirname(os.path.realpath(__file__)) + "/data/kakao.csv", "w", encoding='utf-8', newline='')
    ...
    for s in section_content:
        ...
        if img is not None:
            if img['src'].startswith("/"):
                image = "http://tech.kakao.com" + img['src']
            else:
                image = img['src']
        print(image)
